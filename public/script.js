...
function requestWithdrawal() {
  ...
  if (!address) {
    alert("Please provide a withdrawal address.");
    return;
  }

  if (!password) {
    alert("Password is required for withdrawal.");
    return;
  }

  // Make API call to log withdrawal request
...
function loadWithdrawalHistory() {
  ...
  fetch(`/get-withdrawal-history?userId=${userId}`)
    .then(res => res.json())
    .then(data => {
      ...
      data.forEach(withdrawal => {
        ...
        withdrawalItem.innerHTML = `
          ...
          <p>Requested on: ${withdrawal.request_date}</p>
        `;
        historyContainer.appendChild(withdrawalItem);
      });
    })
    .catch(err => {
      alert("Error fetching withdrawal history: " + err.message);
    });
}

// Call loadWithdrawalHistory only if user is logged in
document.addEventListener("DOMContentLoaded", function () {
  if (isUserLoggedIn()) {
