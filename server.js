...
app.post('/api/emergency-fund/add', (req, res) => {
    ...
    db.get('SELECT balance FROM users WHERE id = ?', [userId], (err, row) => {
        ...
        db.run('UPDATE users SET balance = balance - ? WHERE id = ?', [amount, userId], (err) => {
            ...
            db.run(
                'INSERT INTO emergency_funds (user_id, amount, purpose) VALUES (?, ?, ?)',
                [userId, amount, purpose],
                function (err) {
                    if (err) return res.status(500).json({ error: 'Emergency fund creation failed' });
                    res.json({ success: true, fundId: this.lastID });
                }
            );
        });
    });
});
app.get('/api/emergency-fund/view', (req, res) => {
    const userId = req.query.userId;
    if (!userId) return res.status(400).json({ error: 'Missing userId' });

    db.all('SELECT * FROM emergency_funds WHERE user_id = ? ORDER BY created_at DESC', [userId], (err, rows) => {
        if (err) return res.status(500).json({ error: 'Could not load emergency funds' });
        res.json(rows);
    });
});
// Get user balance
